...
from get_env import get_device_id
...
def train_model(model, criterion, dataloaders, optimizer, scheduler, metrics, bpath, num_epochs=3):
    ...
    device = torch.device("cuda:{}".format(get_device_id()) if torch.cuda.is_available() else "cpu")
    model.to(device)
    ...
    for epoch in range(start_epoch, num_epochs+1):
            ...
            for sample in tqdm(iter(dataloaders[phase])):
                ...
                with torch.set_grad_enabled(phase == 'Train'):
                    ...
                    for name, metric in metrics.items():
                        if name == 'dice' or name == 'dice_lesao':
        ...
        epoch_valid_dice = np.mean(batchsummary['Valid_dice_lesao'])
        is_best = False
