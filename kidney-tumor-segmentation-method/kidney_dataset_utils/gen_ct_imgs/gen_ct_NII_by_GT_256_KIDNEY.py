...
def generate_imgs_masks_2d(path_root, output_dir, local, width, height, only_injuries_slices=False, histogram_specification=False):
    ...
    for folder in folders:
        ...
        if not nome_paciente in ["case_00028"]:
            continue
        ...
        imagem_especificada_array = imagem_array
        # if histogram_specification:
        ...
        sitk.WriteImage(mask_itk, os.path.join(
            output_dir, "mask_"+nome_paciente + ".nii.gz"))

            
        imagem_especificada_array = imagem_especificada_array.transpose(1, 2, 0)
        imaging = sitk.GetImageFromArray(imagem_especificada_array)
        
        sitk.WriteImage(imaging, os.path.join(output_dir, "image_"+nome_paciente + ".nii.gz"))
