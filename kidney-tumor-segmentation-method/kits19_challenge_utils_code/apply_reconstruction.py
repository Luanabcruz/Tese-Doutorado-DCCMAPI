...
output = r'../pipeline_completo_nii/nii_pipeline_completo_lesao_final'
...
for pred in os.listdir(kidney_pred):
    ...
    # ETAPA 1 - interseção do rim reconstruído e lesão

    intersection = np.bitwise_and(
        pred_reconstructed, sitk_pred_array_t_raw)
    ...
    result = np.bitwise_or(intersection, sitk_pred_array_t)
    result = post_processing_3D(np.asarray(result, np.uint32))
