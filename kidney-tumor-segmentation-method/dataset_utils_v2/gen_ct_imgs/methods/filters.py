...
def manter_fatias_com_rins(imagem_array, gt_array):

    imagem_rins = np.zeros(imagem_array.shape)
    np.copyto(imagem_rins, imagem_array, where=(gt_array != 0))
...
def rescale_intensity(imagem_especificada_array, rgb_scale=True):
    ...
    if rgb_scale:
        nova_imagem = filtro.Execute(nova_imagem, 0, 255)
    else:
        nova_imagem = filtro.Execute(nova_imagem, 0, 1)
...
def resize_proporcional_image_2(image, width=256, height=256):

    # image = image.astype(np.float64)
    image = np.where(image == 2, 255, 0)
    ...
    imagens = np.where(imagens > 128, 2, 0)
    imagens = np.asarray(imagens, np.uint8)
